...
const getBestRecords = (records: PersonalRecord[]): PersonalRecord[] => {
    ...
    records.forEach(record => {
        ...
        if (!existingBest) {
            ...
            const bestWeightKg = existingBest.weightUnit === 'lbs' ? existingBest.weight * 0.453592 : existingBest.weight;
            const currentWeightKg = record.weightUnit === 'lbs' ? record.weight * 0.453592 : record.weight;
...
export default function MilestonesPage() {
  ...
  const handleParsedData = (parsedData: ParsePersonalRecordsOutput) => {
    // This function is now primarily for displaying the parsing results.
    // The actual saving and consolidation happens in the server action.
    if (parsedData.records.length > 0) {
      toast({
        title: "Screenshot Processed",
        description: `Found ${parsedData.records.length} record(s). New personal bests have been saved.`,
      });
    } else {
       toast({
        title: "No Records Found",
        description: "The AI could not find any records in the screenshot.",
        variant: "default",
      });
    }
    setActiveForm('none');
