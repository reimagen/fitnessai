...
// --- Migration Logic ---
export const migrateFromLocalStorageToFirestore = async () => {
  ...
  try {
    ...
    if (savedLogsString) {
      const logs: WorkoutLog[] = JSON.parse(savedLogsString);
      const batch = writeBatch(db);
      logs.forEach(log => {
        const newDocRef = doc(workoutLogsCollection);
        // Ensure date is a valid Date object before converting
        const logData = { ...log, date: new Date(log.date) };
        delete (logData as any).dateString; // remove obsolete field
        batch.set(newDocRef, logData);
      });
      await batch.commit();
      localStorage.removeItem(LOCAL_STORAGE_KEY_WORKOUTS);
    }

    // Migrate PRs
    const savedPrsString = localStorage.getItem(LOCAL_STORAGE_KEY_PRS);
    if (savedPrsString) {
      const prs: PersonalRecord[] = JSON.parse(savedPrsString);
      const batch = writeBatch(db);
      prs.forEach(pr => {
        const newDocRef = doc(personalRecordsCollection);
        const prData = { ...pr, date: new Date(pr.date) };
        delete (prData as any).dateString; // remove obsolete field
        batch.set(newDocRef, prData);
      });
      await batch.commit();
    ...
    if (savedProfileString) {
      ...
      const profileWithDates = {
          ...
          fitnessGoals: profile.fitnessGoals.map(g => ({...g, targetDate: g.targetDate ? new Date(g.targetDate) : undefined }))
      };
      await updateUserProfile(profileWithDates);
