...
import type { UserProfile, WorkoutLog, PersonalRecord, StrengthImbalanceOutput, StoredLiftProgressionAnalysis } from "@/lib/types";
...
const constructUserProfileContext = (
    ...
    if (userProfile.strengthAnalysis) {
        context += `- Overall Summary: ${userProfile.strengthAnalysis.result.summary}\n`;
        if (userProfile.strengthAnalysis.result.findings.length > 0) {
            userProfile.strengthAnalysis.result.findings.forEach(finding => {
    ...
    context += "\nLift Progression Analysis Summary:\n";
    if (userProfile.liftProgressionAnalysis && Object.keys(userProfile.liftProgressionAnalysis).length > 0) {
        Object.entries(userProfile.liftProgressionAnalysis).forEach(([exerciseName, analysis]) => {
            context += `- ${exerciseName.charAt(0).toUpperCase() + exerciseName.slice(1)}: Status: ${analysis.result.progressionStatus}. Recommendation: ${analysis.result.recommendation}\n`;
        });
    } else {
        context += "- No lift progression analysis has been performed yet.\n";
    }
...
export default function PlanPage() {
  ...
  const userProfileContextString = useMemo(() => {
    ...
    return constructUserProfileContext(userProfile, workoutLogs, personalRecords);
